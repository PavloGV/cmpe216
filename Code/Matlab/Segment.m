...
classdef Segment < handle
    ...
    methods
        % Class constructor 
        function seg_obj = Segment(origin_position, segment_angle, segment_length, reference_beginning, reference_end)
            if nargin > 0
                seg_obj.ref_joint_pos = reference_beginning;    % Set reference segment starting point (joint)
                seg_obj.ref_end_pos = reference_end;            % Set reference segment end point (foot)
                
                rot = [cos(segment_angle) -sin(segment_angle); sin(segment_angle) cos(segment_angle)];
                
                rotated_seg = rot*(reference_end - reference_beginning) + reference_beginning;
